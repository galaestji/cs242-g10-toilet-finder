...
def init_db():
    ...
    # Ensure legacy database schema is upgraded if needed
...
def seed_real_data():
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT COUNT(*) FROM toilets")
    toilets_count = cursor.fetchone()[0]
    cursor.execute("SELECT COUNT(*) FROM toilet_images")
    ...
    if toilets_count == 0:
        ...
        """, IMAGES)
        conn.commit()
        print("Seeded image data for existing toilets")
    else:
